-- H2K 99 Nights in the Forest - Android KRNL
-- By H2K
...
local function isAnimalOrCultist(obj)
    ...
    local animalNames = {"wolf","bear","rabbit","bunny","deer","boar","pig","animal","mob","hostile","creature","alpha"}
    local cultistNames = {"cultist","enemy","npc","bandit","raider"}

    for _, target in pairs(animalNames) do
        if name:find(target) or parentName:find(target) then return true end
    end
    for _, target in pairs(cultistNames) do
        if name:find(target) or parentName:find(target) then return true end
    end
                    ...
                    pcall(function()
                        ...
                        for _, remote in pairs(ReplicatedStorage:GetDescendants()) do
                            if remote:IsA("RemoteEvent") then
                                ...
                                if rName:find("damage") or rName:find("hit") or rName:find("attack") or rName:find("combat") then
...
local function teleportToCamp()
    ...
    if camp then
        if typeof(camp) == "Vector3" then
            character.HumanoidRootPart.CFrame = CFrame.new(camp)
        else
            character.HumanoidRootPart.CFrame = camp.CFrame + Vector3.new(0,5,0)
        end
    end
end
...
local function createGUI()
    ...
    tpCampBtn.MouseButton1Click:Connect(teleportToCamp)
    openChestBtn.MouseButton1Click:Connect(instaOpenChests)
        ...
        while screenGui.Parent do
            if ScriptState.killAura and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                for _, obj in pairs(Workspace:GetDescendants()) do
                    if isAnimalOrCultist(obj) then
                        local targetRoot = obj:FindFirstChild("HumanoidRootPart") or obj:FindFirstChild("Torso") or obj:FindFirstChildOfClass("Part")
                        if targetRoot and (rootPart.Position - targetRoot.Position).Magnitude <= ScriptState.killAuraRange then
                            count = count + 1
                        end
                    end
                end
