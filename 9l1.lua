...
local function isAnimalOrCultist(obj)
    ...
    local animalNames = {"wolf", "bear", "rabbit", "bunny", "deer", "boar", "pig", "animal", "mob", "hostile", "creature"}
    local cultistNames = {"cultist", "enemy", "npc", "bandit", "raider"}
    
    for _, v in pairs(animalNames) do
        if name:find(v) or parentName:find(v) then return true end
    end
    for _, v in pairs(cultistNames) do
        if name:find(v) or parentName:find(v) then return true end
    end
...
local function performKillAura()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        if isAnimalOrCultist(obj) and obj ~= character then
                    ...
                    pcall(function()
                        if tool then
                            ...
                            for _, remote in pairs(tool:GetDescendants()) do
                                if remote:IsA("RemoteEvent") then
                                    remote:FireServer(targetRoot, targetHum, 999)
                                end
                            end
                        end
                        
                        if targetHum and targetHum.Parent ~= character then
                        ...
                        for _, remote in pairs(ReplicatedStorage:GetDescendants()) do
                            if remote:IsA("RemoteEvent") then
                                ...
                                if remoteName:find("damage") or remoteName:find("hit") or remoteName:find("attack") then
    ...
    spawn(function()
        while screenGui.Parent do
            if ScriptState.killAura and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                for _, obj in pairs(Workspace:GetDescendants()) do
                    if isAnimalOrCultist(obj) and obj ~= LocalPlayer.Character then
                        local