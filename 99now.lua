...
-- Mod completo con Kill Aura, Speed, Infinite Jump, Auto Scrap
...
local function startAutoScrap()
    ...
    Connections.scrapLoop = RunService.Heartbeat:Connect(function()
        ...
        -- Buscar chatarra disponible
        for _, item in pairs(ItemsFolder:GetChildren()) do
            if table.find(scrapItems, item.Name) then
                local part = item.PrimaryPart or item:FindFirstChildWhichIsA("BasePart")
                if part then
                    pcall(function()
                        -- Spoof - traer item sin mover al jugador
                    ...
                    break -- Solo traer uno por ciclo
                end
            end
        end
        
        task.wait(1) -- Timing de 1 segundo entre items
    end)
end
...
local function createModMenu()
    ...
    campBtn.Position = UDim2.new(0, 0, 0, 230)
...
local function antiDetection()
    -- Mover GUIs a CoreGui para mayor protección
    spawn(function()
        task.wait(1)
    ...
    -- Protección adicional contra resets
    spawn(function()
        while task.wait(5) do
            if not game:GetService("CoreGui"):FindFirstChild("H2KIcon") then
                -- Si se perdió el icono, recrear todo el sistema
                icon = createFloatingIcon()
                menu = createModMenu()
                -- Reconectar eventos aquí si es necesario
            end
        end
    end)
end
...
spawn(function()
    while task.wait(1) do
        -- Verificar que todas las funciones activas sigan corriendo
        ...
        -- Mantener velocidad si está activa
...
print("🎯 Features: Kill Aura, Speed x65, Infinite Jump, Insta Chests, Auto Scrap")
